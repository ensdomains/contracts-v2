...
| **Graveyard daemon desync** | Migrated names keep live v1 registry records because the graveyard daemon stalls | Each migration observed on v2 is tracked until the `Graveyard` holds its v1 side — the registry record for unwrapped/unlocked names, the `NameWrapper` token for locked names; sustained lag beyond a threshold alerts (a little desync is expected and tolerated) |
...
| `graveyard-clear-lag` | Migrated names whose v1 side (registry record, or wrapper token for locked names) is verified still live past `--graveyard-lag-hours` (default 24h); also flags names left untracked by the `--graveyard-pending-max` cap | warning |
