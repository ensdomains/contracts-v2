...
import {StandardPricing} from "~test/unit/registrar/StandardPricing.sol";
import {
...
contract StandardRentPriceOracleFixture is Test {
    ...
    function deployStandardRentPriceOracleFixture() public {
        ...
        for (uint256 i; i < paymentTokens.length; ++i) {
            ...
            token.mint(payer, 1e9 * 10 ** token.decimals());
        }
        ...
        for (uint256 i; i < paymentTokens.length; ++i) {
            MockERC20 token = paymentTokens[i];
            //token.approve(payer, type(uint256).max);
            token.approve(address(rentPriceOracle), type(uint256).max);
        }
        vm.stopPrank();
