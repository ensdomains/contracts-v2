...
abstract contract AbstractWrapperReceiver is ERC165, IERC1155Receiver {
    ...
    modifier onlyWrapper() {
        if (msg.sender != address(NAME_WRAPPER)) {
            WrappedErrorLib.wrapAndRevert(abi.encodeWithSelector(UnauthorizedCaller.selector, msg.sender));
        }
        _;
    }
    ...
    function onERC1155BatchReceived(
        ...
        /*amounts*/
        bytes calldata data
    ) external onlyWrapper withData(data, 64 + ids.length * LibMigration.MIN_DATA_SIZE) returns (bytes4) {
    ...
    function finishERC1155Migration(uint256[] calldata ids, LibMigration.Data[] calldata mds) external {
    ...
    function _migrateWrapped(uint256[] calldata ids, LibMigration.Data[] calldata mds) internal virtual;
