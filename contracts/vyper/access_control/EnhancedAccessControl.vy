...
ROOT_RESOURCE_VALUE: constant(uint256) = 0
...
@internal
@pure
def _revert3(sig: String[128], resource: uint256, role_bitmap: uint256, account: address):
    raw_revert(abi_encode(resource, role_bitmap, account, method_id=method_id(sig)))
...
def _effective_roles(resource: uint256, account: address) -> uint256:
    return self._get_roles(ROOT_RESOURCE_VALUE, account) | self._get_roles(resource, account)
...
def _check_can_grant_roles(resource: uint256, role_bitmap: uint256, account: address):
    if role_bitmap & ~self._get_settable_roles(resource, account) != 0:
...
def _check_can_revoke_roles(resource: uint256, role_bitmap: uint256, account: address):
    if role_bitmap & ~self._default_revokable_roles(resource, account) != 0:
...
def grantRoles(resource: uint256, role_bitmap: uint256, account: address) -> bool:
    ...
    if resource == ROOT_RESOURCE_VALUE:
...
def grantRootRoles(role_bitmap: uint256, account: address) -> bool:
    self._check_can_grant_roles(ROOT_RESOURCE_VALUE, role_bitmap, msg.sender)
    return self._grant_roles(ROOT_RESOURCE_VALUE, role_bitmap, account, True)
...
def revokeRootRoles(role_bitmap: uint256, account: address) -> bool:
    self._check_can_revoke_roles(ROOT_RESOURCE_VALUE, role_bitmap, msg.sender)
    return self._revoke_roles(ROOT_RESOURCE_VALUE, role_bitmap, account, True)

@external
@view
def ROOT_RESOURCE() -> uint256:
    return ROOT_RESOURCE_VALUE
...
def hasRootRoles(role_bitmap: uint256, account: address) -> bool:
    return self._get_roles(ROOT_RESOURCE_VALUE, account) & role_bitmap == role_bitmap
...
def hasAssignees(resource: uint256, role_bitmap: uint256) -> bool:
    mask: uint256 = self._role_bitmap_to_mask(role_bitmap)
    return self._role_count[resource] & mask != 0
